...
module Market = 
    ...
    let loadItems (items:Refined list) = 
        [
        ...
        |> List.map (fun (item:Refined) -> TypeId item)
        |> List.map (fun (itemid:int)   -> itemid, loadItem itemid Jita )
...
//        let LoadItem item = LoadMarketSnapshot location quantity item
//
//        [
...
//        let priceMap = LoadIceProductPrices (SystemId Jita) 1
//                       |> List.map action
