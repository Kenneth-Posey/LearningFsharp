...
module Samples.DataAccess.AzureMarketPlace
...
let dummy() = ()
[<Generate>]
type Lawyers = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/LexisNexis/ConsumerLegalArticles/", LocalSchemaFile="ConsumerLegalArticles.csdl", ForceUpdate=false >

[<Category("DataAccess.AzureMarketPlace");
...
let dummy1() = ()
[<Generate>]
type MarketBank = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/RpmConsulting/MarketBank/", LocalSchemaFile="MarketBank.csdl", ForceUpdate=false>
...
let MarketBank() = 
    ...
    for i in results do 
        printfn "%A - %A" i.Id i.AnnuitiesPenetrationIndex
...
let dummy2() = ()
[<Generate>]
type BankInfo = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/RpmConsulting/BranchInfo", LocalSchemaFile="BranchInfo.csdl", ForceUpdate=false>
...
let BranchInfo() = 
    ...
    for i in results do 
        printfn "%A - %A" i.BranchName i.BranchDeposits
...
let dummy4() = ()
[<Generate>]
type C = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/data.gov/Crimes", LocalSchemaFile="Crimes.csdl", ForceUpdate=false>
...
let DataGov() = 
    ...
    // Not all query operations can be performed on the server, so we do some
    // in-memory transformations after performing the query.
    let results =
        resultsOfRemoteQuery
...
let dummy5() = ()
[<Generate>]
type D = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = "https://api.datamarket.azure.com/Esri/KeyUSDemographicsTrial/", LocalSchemaFile="KeyUSDemographicsTrial.csdl", ForceUpdate=false>
...
let dummy6() = ()
[<Generate>]
type L = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/LexisNexis/LegalCommunities/", LocalSchemaFile="LegalCommunities.csdl", ForceUpdate=false>
...
let dummy7() = ()
[<Generate>]
type SME = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/Chanjet/ChinaSmallAndMediumEnterprises/", LocalSchemaFile="ChinaSmallAndMediumEnterprises.csdl", ForceUpdate=false>
...
let Chanjet() = 
    ...
    for i in results do 
        printfn "%A - %A" i.ProvinceNameEnglish i.ProvinceNameChinese
...
let dummy8() = ()
[<Generate>]
type CPI = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/MetricMash/USConsumerPriceIndex/", LocalSchemaFile="USConsumerPriceIndex.csdl", ForceUpdate=false>
...
let MetricMash() = 
    ...
    for i in results do 
        printfn "%A - %A" i.DateValue i.Index
...
let dummy9() = ()
[<Generate>]
type EHR = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = @"https://api.datamarket.azure.com/EDR/EnvironmentalHazardRank", LocalSchemaFile="EnvironmentalHazardRank.csdl", ForceUpdate=false>
...
let EDR() = 
    ...
    for i in results do 
        printfn "%A - %A" i.ZipCode i.EnviromentalHazardIndex
...
let dummy11() = ()
[<Generate>]
type Unemployment = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = "https://api.datamarket.azure.com/MetricMash/USUnemploymentData/", LocalSchemaFile="USUnemploymentData.csdl", ForceUpdate=false>
...
let MetricMash1() = 
    ...
    for i in results do 
        printfn "%A - %A" (i.MonthMMM + ", " + i.Year) i.Value
...
let dummy12() = ()
[<Generate>]
type Util = Microsoft.FSharp.Data.TypeProviders.ODataService<ServiceUri = "https://api.datamarket.azure.com/Microsoft/UtilityRateService/", LocalSchemaFile="UtilityRateService.csdl", ForceUpdate=false>
...
let MSU() = 
    ...
    for i in results do 
        printfn "%A - %A" i.Name i.RateSource
