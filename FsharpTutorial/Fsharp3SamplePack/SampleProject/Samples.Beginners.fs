...
[<Support.Helper.SampleAttributes.Sample("Basic")>]
module Samples.Beginners
...
[<Category("Basic Data Types");
  Title("string definition and output");
  Description("This sample shows how to define a string")>]
let Sample42() =
    let str = "my string"
    printfn "my string = %s" str    
    let str2 = """a string with "quotes" in middle"""
    printfn "my second string = %s" str2
    let str3 = @"a string with ""quotes"" in middle"
    printfn "my third string = %s" str3
//--------------------------------------------------------
...
[<Category("Arrays, Hash Tables and Dictionaries");
  Title("Using arrays");
  Description("Using arrays sample")>]
let SampleArray1() =
    let size = 1000 
...
[<Category("Defining Types");
  Title("Basic Discriminated Unions wiht RequireQualifiedAccess");
  Description("Discriminated unions Use RequireQualifiedAccess")>]
let UnionSample0() =
    let r = DU.Case1(1)
    let r2 = DU.Case2(2.)
    printfn "RequireQualifiedAccess requires use DU type name"
...
[<Support("RecordSample2")>]
let dummy42() = ()
type simpleRecord2 = {Name: string; Job: string; Wage: int}
[<CLIMutable>]
type simpleRecord2CLIMutable = {Name: string; Job: string; Wage: int}

[<Category("Defining Types");
  Title("Record with CLIMutable attribute");
  Description("Examples of using CLIMutable attribute")>]
let RecordSample3() =
    let worker = { simpleRecord2.Name = "Bill"; Job = "Cashier"; Wage = 20 }
    let worker2 = { simpleRecord2CLIMutable.Name = "Bill"; Job = "Cashier"; Wage = 20 }
    let checkCtor worker = 
        let ctors = worker.GetType().GetConstructors();
        let ctorNum = ctors |> Seq.exists (fun ctor -> ctor.GetParameters().Length = 0)
        ctorNum
    printfn "find parameterless constructor on workder? %A" (checkCtor worker)
    printfn "find parameterless constructor on workder2? %A" (checkCtor worker2)
...
[<Category("Defining Types");
  Title("Constructing Class with auto property");
  Description("This sample shows how to construct a class with auto property.")>]
let AIPSample() =
    let p = MyPoint()
    printfn "my point is originally located at (%d, %d)" p.X p.Y
    p.X <- 10
    printfn "my point is moved to (%d, %d)" p.X p.Y
...
[<Category("Defining Types");
  Title("Define a class with mutable variable");
...
[<Category("Disposal");
  Title("Create disposal function");
...
[<Category("Defining Operator");
  Title("Define operator - inline function");
...
[<Support("DefinteOperator3")>]
let rec hcf a b =
  if b = 0u then failwith "b cannot be 0"
  elif a = 0u then b
